...
class SkywalkerGrabSceneCfg(InteractiveSceneCfg):
    ...
    """
    Creates a Surface Gripper and attaches it to the specified parent path.
    """
    # # Create a new SurfaceGripper
    # properties = surface_gripper.Surface_Gripper_Properties(
    #     parentPath=parent_path,
    #     translate=0.1,  # Apply a slight offset
    #     direction="x",
    #     gripThreshold=0.01,
    #     forceLimit=1000000.0,
    #     torqueLimit=10000.0,
    #     bendAngle=0.1308996938995747,
    #     kp=100.0,
    #     kd=100.0,
    #     disableGravity=True
    # )
    
    # gripper = surface_gripper.SurfaceGripper()

    #use case
    # Create the environment
    # env = ManagerBasedEnv()

    # # Create the Surface Gripper in the environment
    # surface_gripper_instance = create_surface_gripper(parent_path="/World/xarm7/link7")

    # # Check if gripper was successfully created
    # if surface_gripper_instance:
    #     # Create the action term for the surface gripper
    #     cfg = actions_cfg.SurfaceGripperActionCfg()  # Define the action config
    #     action_term = SurfaceGripperActionTerm(cfg, env, surface_gripper_instance)

    #     # Now you can control the gripper's state
    #     actions = torch.tensor([1.0])  # Open gripper (positive value)
    #     action_term.process_actions(actions)

    #     actions = torch.tensor([-1.0])  # Close gripper (negative value)
    #     action_term.process_actions(actions)
...
class TerminationsCfg:
    ...
    object_dropping = DoneTerm(
        func=mdp.root_height_below_minimum, params={"minimum_height": 0.20, "asset_cfg": SceneEntityCfg("object")}
    )
@configclass
class CurriculumCfg:
