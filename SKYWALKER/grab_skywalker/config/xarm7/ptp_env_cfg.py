...
class CommandsCfg:
    ...
    target_pose = UniformPoseCommandCfg(
        ...
        resampling_time_range=(7.0, 7.0),  # Match episode length (tripled)
        debug_vis=True,
        ranges=UniformPoseCommandCfg.Ranges(
            pos_x=(0.2, 0.35),   # Forward reach range
            pos_y=(-0.4, 0.4),  # Left-right reach range
            pos_z=(0.30, 0.50),  # Height range above base (robot at 0.25m + reach height)
            roll=(3.05, 3.25),     # π radians (180°) - flip Z-axis direction
            pitch=(-2.1, -1.4),  # -π/2 radians (-90°) - X-axis pointing upward
            yaw=(-1.1, 1.1),     # Small yaw variation
        ),
    )
...
class PTPEnvCfg(ManagerBasedRLEnvCfg):
    ...
    # Basic settings
    episode_length_s = 7.0  # 21 seconds episode length (tripled from 7s for more learning time)
    decimation = 2
...
class PTPEnvCfg_PLAY(PTPEnvCfg):
    def __post_init__(self):
        ...
        # longer episodes for more comprehensive testing
