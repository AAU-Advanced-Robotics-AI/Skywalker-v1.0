...
# in BaseMoveSceneCfg
    target_pose_marker = RigidObjectCfg(
        ...
        init_state=RigidObjectCfg.InitialStateCfg(pos=(0.0, 0.0, 0.55), rot=(1, 0, 0, 0)),
    )
...
class CommandsCfg:
     ...
     target_pose = UniformPoseCommandCfg(
        ...
        ranges=UniformPoseCommandCfg.Ranges(
            ...
            pos_y=(-0.35, 0.35),  # All subsequent spawns handled by our functions
...
class EventsCfg:
    ...
    # DEBUG: Print anchor position for height analysis
    debug_anchor_position = EventTerm(
        func=bm.debug_print_anchor_position,
        mode="interval",
        interval_range_s=(5.0, 5.0),   # Print every 5 seconds
        params={
            "print_anchor": True,  # Just a dummy parameter to satisfy the requirement
        },
    )
    ...
    # Fix command heights to match base height
    fix_command_heights = EventTerm(
        func=bm.fix_command_heights_to_base_event,
        mode="interval",
        interval_range_s=(0.2, 0.2),   # update after commands are sampled  
        params={
            "command_name": "target_pose",
...
class RewardsCfg:
    ...
    position_tracking_fine = RewTerm(
        ...
        params={"std": 0.10, "command_name": "target_pose", "ee_body": "link_eef"},
    )
    ...
    orientation_align = RewTerm(
        ...
        params={"command_name": "target_pose", "std_deg": 10., "gate_with_distance": True, "gate_sigma": 0.15},
    )   
    ...
    base_upright_pen = RewTerm(
        func=bm.base_upright_penalty, weight=-500.0,
        params={"roll_limit": math.radians(1.0), "pitch_limit": math.radians(1.0)},
    )
    
    base_lift_pen = RewTerm(
        func=bm.base_lift_penalty, weight=-500.0,  # Much stronger penalty
        params={"z_nom": 0.501, "deadband": 0.0005},  # Tighter tolerance: 0.5mm
    )
    
    # Additional vertical velocity penalty to discourage bouncing
    base_vertical_velocity_pen = RewTerm(
        func=bm.base_vertical_velocity_penalty, weight=-70.0  # Increased from -10.0
    )
