...
- Builds the 35-D observation exactly like NEW base_move training:
  [joint_pos(7), joint_vel(7), base_pos_w(3), base_quat_w(4), target_pose_command(7), actions(7)]
- Uses WORLD COORDINATES for better sim-to-real transfer (no EE dependency).
...
class BaseMoveController:
    ...
    def __init__(self):
        ...
        self.target_position = [0.00, -0.1, 0.0]        # In front of robot (negative X in base frame)
...
def main():
    ...
    warmup_time = 1.0  # seconds - restored for stability
    ...
    def build_base_move_obs():
        ...
        🚨 UPDATED FOR WORLD COORDINATES: Better sim-to-real transfer!
        - base_pos_w  = ObsTerm(func=bm.base_position_w)   # Base position in world
        - base_quat_w = ObsTerm(func=bm.base_orientation_w) # Base orientation in world
        ...
        # 🎯 WORLD COORDINATE observations using ACTUAL training functions!
        base_pos_w = bm.base_position_w(mock_env)      # (1,3) - EXACT training function!
        base_quat_w = bm.base_orientation_w(mock_env)  # (1,4) - EXACT training function!
        ...
        obs = torch.cat([
            joint_pos,           # 7
            joint_vel,           # 7  
            base_pos_w,          # 3 - 🎯 NEW: World coordinate base position
            base_quat_w,         # 4 - 🎯 NEW: World coordinate base orientation
    ...
    try:
        while simulation_app.is_running() and controller.running:
            ...
            if current_step % 50 == 0:
                ...
                print(f"   🌍 Base Position World [14:17]: {obs_np[14:17]}")
                print(f"   🧭 Base Quaternion World [17:21]: {obs_np[17:21]}")
            ...
            #print(f"   🎯 Default joint pos (first 7): {q_default[0, :7].cpu().numpy()}")
            ...
            if step_count % 50 == 0:  # More frequent debugging
                ...
                # 🚨 CRITICAL CHECK: Let's debug the actual positions and orientations
                # Get actual base position and orientation in world coordinates using robot's find_bodies method
                base_body_ids, _ = robot.find_bodies("link_base")
                ee_body_ids, _ = robot.find_bodies("link_eef")  # EE body name
                ...
                base_quat_world = robot.data.body_quat_w[0, base_body_idx, :].cpu().numpy()  # [w,x,y,z]
                ee_pos_world = robot.data.body_pos_w[0, ee_body_idx, :3].cpu().numpy()
                ee_quat_world = robot.data.body_quat_w[0, ee_body_idx, :].cpu().numpy()  # [w,x,y,z]
                
                # Convert quaternions to Euler angles (roll, pitch, yaw in degrees) for readability
                base_euler = Rotation.from_quat([base_quat_world[1], base_quat_world[2], base_quat_world[3], base_quat_world[0]]).as_euler('xyz', degrees=True)
                ee_euler = Rotation.from_quat([ee_quat_world[1], ee_quat_world[2], ee_quat_world[3], ee_quat_world[0]]).as_euler('xyz', degrees=True)
                
                print(f"   🌍 BASE in World Coordinates:")
                print(f"      Position: [{base_pos_world[0]:.3f}, {base_pos_world[1]:.3f}, {base_pos_world[2]:.3f}]")
                print(f"      Quaternion [w,x,y,z]: [{base_quat_world[0]:.3f}, {base_quat_world[1]:.3f}, {base_quat_world[2]:.3f}, {base_quat_world[3]:.3f}]")
                print(f"      Euler [R,P,Y] (deg): [{base_euler[0]:.1f}, {base_euler[1]:.1f}, {base_euler[2]:.1f}]")
                
                print(f"   🤖 END EFFECTOR in World Coordinates:")
                print(f"      Position: [{ee_pos_world[0]:.3f}, {ee_pos_world[1]:.3f}, {ee_pos_world[2]:.3f}]")
                print(f"      Quaternion [w,x,y,z]: [{ee_quat_world[0]:.3f}, {ee_quat_world[1]:.3f}, {ee_quat_world[2]:.3f}, {ee_quat_world[3]:.3f}]")
                print(f"      Euler [R,P,Y] (deg): [{ee_euler[0]:.1f}, {ee_euler[1]:.1f}, {ee_euler[2]:.1f}]")
