...
# Import omni and USD for cylinder_self_grasp_penalty function
import omni.usd
...
def forward_progress(env, w: float = 1.0):
    ...
    curr_x = robot_pos[:, 0]  # (N,) - current X positions
    
    # Initialize or get previous X positions
    if not hasattr(env, "_prev_x"):
        env._prev_x = curr_x.clone()
        return torch.zeros_like(curr_x)  # No reward on first call
    
    # Calculate forward progress (positive X movement only)
    delta_x = curr_x - env._prev_x  # (N,) - change in X position
    forward_movement = torch.clamp(delta_x, min=0.0)  # Only positive movement
    
    # Reset previous positions on environment reset
    if hasattr(env, "reset_buf"):
        done = env.reset_buf.bool()
        env._prev_x[done] = curr_x[done]  # Reset tracking for done environments
        forward_movement[done] = 0.0  # No reward on reset
    
    # Update previous positions for next timestep
    env._prev_x = curr_x.clone()
    
    return w * forward_movement  # (N,) - reward for forward X movement
...
def skywalker_reward(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    # --- predicates
    # Check grasping from both EE and base grippers
    def check_grasping(cube_idx):
        # Check EE gripper
        ee_grasp = is_grasping_fixed_object(
            env,
            ee_cfg=SceneEntityCfg("ee_frame"),
            cube_cfg=SceneEntityCfg(f"cube{cube_idx+1}"),
            tol=_TOL_GRASP,
            grip_term="gripper_action"
        ).bool()
        
        # Check base gripper (cylinder surface gripper)
        base_grasp = is_grasping_fixed_object(
            env,
            ee_cfg=SceneEntityCfg("cylinder_SG_frame"),
            cube_cfg=SceneEntityCfg(f"cube{cube_idx+1}"),
            tol=_TOL_GRASP,
            grip_term="gripper_action2"  # Use gripper_action2 for base gripper
        ).bool()
        
        # Return True if either gripper is grasping
        return ee_grasp | base_grasp
    
    grasped = lambda k: check_grasping(k)

    # Handle dummy gripper action (no is_closed method for JointPositionAction)
    try:
        clamp_closed = env.action_manager.get_term("gripper_action2").is_closed().bool()
    except AttributeError:
        # For dummy gripper action, assume always closed for compatibility
        clamp_closed = torch.ones(env.num_envs, dtype=torch.bool, device=env.device)
    ...
    m = (phase == 0) & grasped(0).flatten()
    env._phase[m] = 1; evt |= m

    m = (phase == 1) & grasped(0).flatten() & at_dock
    env._phase[m] = 2; evt |= m

    m = (phase == 2) & at_dock & (~grasped(0).flatten())
    env._phase[m] = 3; evt |= m

    m = (phase == 3) & grasped(1).flatten() & at_dock
    env._phase[m] = 4; evt |= m

    m = (phase == 4) & grasped(1).flatten() & (~at_dock)
    env._phase[m] = 5; evt |= m

    m = (phase == 5) & at_goal & grasped(1).flatten()
    ...
    # extra shaping
    dense += drag_shaping_if_grasped(base, dock, grasped(0).flatten(), phase == 1, k_val=0.55)
    dense += drag_shaping_if_grasped(base, goal, grasped(1).flatten(), phase == 4, k_val=0.55)
    ...
    try:
        ...
        # For dummy gripper action, assume always closed for compatibility
        grip_closed = torch.ones(env.num_envs, dtype=torch.float, device=env.device)
    ...
    if env._debug_counter % 50 == 0:  # Print every 50 steps instead of every 100 steps
        # Calculate distances for spatial awareness
        ee_to_cube1_dist = torch.norm(ee - cubes[0, idx], dim=1)
        ee_to_cube2_dist = torch.norm(ee - cubes[1, idx], dim=1)
        base_to_dock_dist = torch.norm(base - dock, dim=1)
        base_to_goal_dist = torch.norm(base - goal, dim=1)
        
        # Show distances for first few environments
        num_envs_to_show = min(4, env.num_envs)
        
        print(f"[debug] step {env._debug_counter}: phase[0]={phase[0]}, at_dock[0]={at_dock[0].item()}")
        print(f"  EE→cube1 dist: {ee_to_cube1_dist[:num_envs_to_show].tolist()}")
        print(f"  EE→cube2 dist: {ee_to_cube2_dist[:num_envs_to_show].tolist()}")
        print(f"  Base→dock dist: {base_to_dock_dist[:num_envs_to_show].tolist()}")
        print(f"  Base→goal dist: {base_to_goal_dist[:num_envs_to_show].tolist()}")
...
def is_grasping_fixed_object(
    ...
    """Check if the gripper is closed using actual surface gripper state.
    
    Now uses real SurfaceGripperActionCfg instead of dummy joint actions:
    - gripper_action: EE_SG surface gripper for object manipulation
    - gripper_action2: Cylinder_SG surface gripper for ground friction
    """
    try:
        gripper = env.action_manager.get_term(grip_term)
        
        # Use the actual surface gripper's is_closed() method
        if hasattr(gripper, 'is_closed'):
            is_closed = gripper.is_closed().float()
        else:
            # Fallback to always closed if method not available
            is_closed = torch.ones(env.num_envs, dtype=torch.float, device=env.device)
    ...
    # For FrameTransformer, use target_pos_w with correct indexing
    ...
    # Debug output to understand positioning (commented out to reduce output)
    # if env.num_envs > 0:
    #     print(f"[DEBUG] '{cube_cfg.name}' → ee_pos={ee_pos[0].tolist()}, cube_pos={cube_pos[0].tolist()}")
    #     print(f"[DEBUG] '{cube_cfg.name}' → distance={dist[0].item():.3f}, tolerance={tol}, is_near={is_near[0].item()}")
...
def simultaneous_gripper_penalty(
    ...
    try:
        ...
        # For dummy gripper action, assume always closed for compatibility
        g1_closed = torch.ones(env.num_envs, dtype=torch.float, device=env.device)
        g2_closed = torch.ones(env.num_envs, dtype=torch.float, device=env.device)

    # Handle dummy gripper timer (may not exist for JointPositionAction)
    try:
        timers = term1._shared_gripper_state_timer     # shared int32 counter
    except AttributeError:
        # For dummy gripper action, simulate grace period as always passed
        timers = torch.full((env.num_envs,), grace_steps + 1, dtype=torch.int32, device=env.device)
...
def cylinder_self_grasp_penalty(
    ...
    try:
        ...
        # For dummy gripper action, assume always closed for compatibility
        closed = torch.ones(env.num_envs, dtype=torch.float, device=env.device)
...
def wall_proximity_penalty(
    ...
    try:
        ...
        # For dummy gripper action, assume always closed for compatibility
        closed = torch.ones(env.num_envs, dtype=torch.bool, device=env.device)
...
def robot_reached_goal(
    ...
    """Binary reward for the object reaching the goal position.
    ...
    """
    # Get the desired goal position from the command manager
    command = env.command_manager.get_command(command_name)
    des_pos_b = command[:, :3]
    des_pos_w = convert_position_to_world_frame(des_pos_b, env, robot_cfg)
    
    # Get the current object position
    object = env.scene[object_cfg.name]
    curr_pos_w = object.data.root_pos_w[:, :3]
    
    # Compute the distance between the current and desired position
...
def convert_position_to_world_frame(pos_b: torch.Tensor, env: ManagerBasedRLEnv, robot_cfg: SceneEntityCfg) -> torch.Tensor:
    """Convert a position from the robot's base frame to the world frame."""
    robot = env.scene[robot_cfg.name]
    robot_pos_w = robot.data.root_pos_w[:, :3]
    robot_quat_w = robot.data.root_quat_w
    
    # Simple approximation - add robot position (ignoring rotation for now)
    pos_w = pos_b + robot_pos_w
    return pos_w


def cylinder_stability_reward(
    env,
    robot_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    gripper_term: str = "gripper_action2",  # cylinder surface gripper
    velocity_threshold: float = 0.1,  # m/s
) -> torch.Tensor:
    """Reward for using cylinder surface gripper to maintain stability.
    
    Encourages the robot to:
    1. Engage the cylinder surface gripper when moving slowly or stationary
    2. Maintain position stability by using ground contact
    
    Args:
        env: The environment.
        robot_cfg: Robot configuration.
        gripper_term: Name of cylinder surface gripper action term.
        velocity_threshold: Velocity below which stability is rewarded.
    ...
    # Get robot base velocity (linear)
    base_velocity = torch.norm(robot.data.root_lin_vel_w[:, :2], dim=1)  # XY velocity only
    
    # Check if cylinder gripper is engaged
    try:
        gripper = env.action_manager.get_term(gripper_term)
        gripper_closed = gripper.is_closed() if hasattr(gripper, 'is_closed') else torch.zeros(env.num_envs, device=env.device)
        
        # Enhanced debug output every 50 steps
        if hasattr(env, '_step_count') and env._step_count % 50 == 0:
            robot_z_pos = robot.data.root_pos_w[:, 2]  # Z position (height)
            print(f"  Cylinder Gripper - Engaged: {gripper_closed[0].item():.1f}, "
                  f"Velocity: {base_velocity[0].item():.3f}m/s, "
                  f"Robot Height: {robot_z_pos[0].item():.3f}m")
                  
    except (KeyError, AttributeError):
        gripper_closed = torch.zeros(env.num_envs, device=env.device)
    
    # Reward when robot is moving slowly/stationary AND cylinder gripper is engaged
    low_velocity = (base_velocity < velocity_threshold).float()
    stability_reward = low_velocity * gripper_closed
    
    return stability_reward


def anti_slip_reward(
    env,
    robot_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    gripper_term: str = "gripper_action2",  # cylinder surface gripper
    position_tolerance: float = 0.05,  # meters
) -> torch.Tensor:
    """Reward for preventing unwanted sliding/slipping using cylinder surface gripper.
    
    This reward encourages the robot to engage the cylinder surface gripper
    when it wants to maintain its current position.
    
    Args:
        env: The environment.
        robot_cfg: Robot configuration.
        gripper_term: Name of cylinder surface gripper action term.
        position_tolerance: Tolerance for position changes.
    ...
    if not hasattr(env, '_prev_robot_pos'):
        env._prev_robot_pos = robot.data.root_pos_w[:, :2].clone()
        return torch.zeros(env.num_envs, device=env.device)
    
    # Calculate position change
    current_pos = robot.data.root_pos_w[:, :2]
    position_change = torch.norm(current_pos - env._prev_robot_pos, dim=1)
    ...
    # Reward when gripper is closed and position is stable
    position_stable = (position_change < position_tolerance).float()
    anti_slip = gripper_closed * position_stable
    
    # Update previous position
    env._prev_robot_pos = current_pos.clone()
