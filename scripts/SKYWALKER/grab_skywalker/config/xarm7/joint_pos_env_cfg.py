...
from isaaclab.assets import ArticulationCfg, AssetBaseCfg, RigidObjectCfg, RigidObjectCollectionCfg, SurfaceGripperCfg
...
class SkywalkerGrabEnvCfg(GrabEnvCfg):
    def __post_init__(self):
        ...
        # switch robot to XARM7 (no fixed base - surface grippers will provide stability)
        ...
        self.actions.gripper_action = mdp.SurfaceGripperActionCfg(
            asset_name="robot",
            gripper_name="ee_gripper",  # Must match scene gripper name
        )
        
        # Cylinder Surface Gripper action - controls the cylinder surface gripper for ground stability
        self.actions.gripper_action2 = mdp.SurfaceGripperActionCfg(
            asset_name="robot", 
            gripper_name="cylinder_gripper",  # Must match scene gripper name
        )
        ...
        # Surface gripper frame transformers
        # Track the rigid bodies that surface grippers are attached to, not the grippers themselves
        self.scene.cylinder_SG_frame = FrameTransformerCfg(
            ...
            target_frames=[
                FrameTransformerCfg.FrameCfg(
                    prim_path="{ENV_REGEX_NS}/Robot/xarm7/link3",  # Track the rigid body, not the surface gripper
        ...
        self.scene.EE_SG_frame = FrameTransformerCfg(
            ...
            target_frames=[
                FrameTransformerCfg.FrameCfg(
                    prim_path="{ENV_REGEX_NS}/Robot/xarm7/link7",  # Track the end-effector rigid body, not the surface gripper
                    offset=OffsetCfg(pos=[0.0, 0.0, 0.0], rot=[0, 0, 0, 1]),
                ),
            ],
        )
        
        # Note: Surface grippers that are part of the robot USD should NOT be configured as separate scene assets
        # They will be automatically detected and accessible through the robot's surface gripper interface
        # when using SurfaceGripperActionTerm
