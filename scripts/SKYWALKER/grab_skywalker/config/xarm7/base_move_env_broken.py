...
class BaseMoveSceneCfg(InteractiveSceneCfg):
    ...
    robot = XARM7_CFG.replace(
        ...
        init_state=ArticulationCfg.InitialStateCfg(pos=(-0.5, 0.0, 0.0)),  # Start robot further back
...
class EventsCfg:
    ...
    # Use smart reset that positions everything correctly before creating weld
    init_grounded_pose = EventTerm(
        func=bm.ground_upright_smart_weld_event,  # NEW: smarter positioning and welding
        mode="reset",
        params={
            "base_xy": (0.0, 0.0),
            "base_z": 0.25,
