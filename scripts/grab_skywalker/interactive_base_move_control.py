...
def base_position_relative_to_ee(robot):
    ...
    try:
        ...
        ee_indices = robot.find_bodies("link7")  # End-effector link
...
class BaseMoveController:
    ...
    def __init__(self):
        ...
        self.target_position = [0.1, 0.2, 0.0]        # XY movement relative to base
        self.target_orientation = [0.0, 0.0, 0.1]     # RPY (rad) - small yaw
    ...
    def create_command_file(self):
        with open(self.command_file, "w") as f:
            ...
            f.write("#   goto X Y YAW  (e.g., goto 0.2 -0.1 0.3)\n")
            f.write("#   quit\n\n")
            f.write("# Ranges (typical training workspace):\n")
            f.write("#   X: -0.2..0.2,  Y: -0.5..0.5,  YAW: -0.4..0.4\n")
...
def main():
    ...
    def build_base_move_obs():
        ...
        # Target pose command (position + quaternion, like training)
        target_pos = torch.tensor([controller.target_position], device=robot.device, dtype=torch.float32)  # (1,3)
        ...
        target_pose_command = torch.cat([target_pos, target_quat], dim=1)  # (1,7)

        # Update target marker visualization (need absolute position for marker)
        base_pos_w, base_quat_w = get_base_pose_world_for_marker(robot)  # Get absolute base pose
        marker_pos = base_pos_w + target_pos  # Show absolute target position
        try:
